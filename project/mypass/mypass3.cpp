    ...
    bool runOnFunction(Function &F) override
    {
      ...
      for (auto &BB : F)
      {
        ...
        for (auto &I : BB)
        {
          ...
          { // AllocaInst? someone could look into this
            // excluded cases: masked/mask loads, not yet masked, ...
            for (auto visited : maskedLoads)
              if (visited == load)
                continue;
            ...
            for (auto U : load->users())
            {
              ...
              if ((userI) && (userI != ldMaskInst))
              {
                for (int i = 0; i < userI->getNumOperands(); ++i)
                  if (load == userI->getOperand(i))
                    userI->setOperand(i, ldMaskInst);
              }
            }
